...
-- 2. 이미 인증 절차가 진행중인 전화번호가 있다면 해당 전화번호에 대한 인증번호를 수정하는 작업으로 대체
...
-- 고객 테이블에서 페이지 수에 해당하는 위치부터 10개의 고객 수를 내림차순으로 반환
...
order by customer_number desc limit 0, 10;
...
-- 필요 추가 데이터 : 고객번호 (화면에서), 로그인 사용자 아이디 (화면에서)
...
-- 고객 테이블에서 특정 고객을 조회하여 반환, 담당자 이름을 반환하기 요양사 테이블을 조인
...
-- 절차상 사용한 용품의 개수 감소 (사용하는 용품이 있을 경우에만)
